...
const jadibot = async (hardi, m, from) => {
      ...
      client[from].ev.on("connection.update", async up => {
        ...
        if (connection === "close") {
          if (reason === DisconnectReason.restartRequired) {
            ...
            return hardi.sendMessage(from, { text: "Anda sudah tidak lagi menjadi bot." })
          }
        }
      })
...
async function stopjadibot(hardi, from) {
  if (!Object.keys(client).includes(from)) {
    return hardi.sendMessage(from, { text: "Anda tidak ada di list jadi bot." })
  }
  try {
...
async function listjadibot(hardi, m) {
  ...
  return hardi.sendMessage(from, { text: text.trim(), mentions, })
}
